...
/**
 * Get CSRF Token (Admin)
 * Returns CSRF token for use in subsequent admin requests
 * Token is also set in X-CSRF-Token header and XSRF-TOKEN cookie
 * GET /api/admin/csrf-token
 */
const getAdminCSRFToken = asyncHandler(async (req, res) => {
 ...
 * - role: Filter by role - 'user', 'host', 'admin' (optional, if not provided shows ALL users)
...
const listUsers = asyncHandler(async (req, res) => {
  ...
  const role = req.query.role; // 'user', 'host', 'admin', or undefined for ALL
  ...
  const usersWithBalance = users.map(user => ({
    ...user,
    balanceINR: balanceMap.get(user._id.toString()) || 0
  }));
 ...
 *
 * Returns: users, top-up (INR + GC), active lobbies, prize pool distributed,
 * host fee paid, platform + caster fee (platform profit for server etc.)
 */
const getDashboardStats = asyncHandler(async (req, res) => {
  ...
  const [
    ...
    // User top-up in INR (successful, user-added)
    WalletHistory.aggregate([
      { $match: { type: 'topup', status: 'success', addedBy: 'user' } },
      { $group: { _id: null, total: { $sum: '$amountINR' } } }
    ]),
    // User top-up in GC (same – for in-app consistency)
    WalletHistory.aggregate([
      { $match: { type: 'topup', status: 'success', addedBy: 'user' } },
      { $group: { _id: null, total: { $sum: '$amountINR' } } }
    ]),
    // Rewards paid to players (prize pool distributed to winners)
    WalletHistory.aggregate([
      { $match: { type: 'reward' } },
      { $group: { _id: null, total: { $sum: '$amountINR' } } }
    ]),
    // Host fee paid (topup addedBy system = host fee credits)
    WalletHistory.aggregate([
      { $match: { type: 'topup', status: 'success', addedBy: 'system' } },
      { $group: { _id: null, total: { $sum: '$amountINR' } } }
    ]),
    // Active lobbies (currently running)
    Tournament.countDocuments({ status: 'running' }),
    // From completed lobbies: platform + caster fee (platform profit)
  ...
  const totalDeposits = totalDepositsINR[0]?.total || 0;
  const totalTopupGCVal = totalTopupGC[0]?.total || 0;
  ...
  const totalFeesFromCompletedLobbies = platformFeeCollected + casterFeeCollected + totalHostFeeFromLobbies;

  res.success(HTTP_STATUS.OK, 'Dashboard statistics retrieved successfully', {
    totalUsers,
    totalDepositsINR: totalDeposits,
    totalTopupGC: totalTopupGCVal,
    activeLobbyCount,
    ...
    casterFeeCollected,
    platformProfit,
    // Fees breakdown from completed lobbies (BR/LW/CS – includes Clash Squad 5/10 host, rest to platform)
    feesBreakdown: {
      platformFeeGC: platformFeeCollected,
      casterFeeGC: casterFeeCollected,
      hostFeeGC: totalHostFeeFromLobbies,
      totalFeesGC: totalFeesFromCompletedLobbies,
      winnerPoolPaidGC: winnerPoolFromLobbies
    },
    // Legacy keys for backward compatibility
    totalDeposits: totalDeposits,
    totalRewards: prizePoolDistributed,
    netProfit: totalDeposits - prizePoolDistributed
  });
});
