/**
 * JWT auth for host SSE where EventSource cannot set Authorization (browser).
 * Accepts `Authorization: Bearer <token>` or `?access_token=<token>` (host role only).
 */
...
const authenticateHostSse = async (req, res, next) => {
  ...
  try {
    ...
    if (user.role !== 'host') return deny(403, 'Forbidden: host only');
...
module.exports = { authenticateHostSse };
